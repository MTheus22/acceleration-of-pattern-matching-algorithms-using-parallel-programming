...
\textbf{Strong Scalability}: Measures how execution time varies with an increasing number of processors for a fixed total problem size. The objective is to maintain relatively constant efficiency as \texttt{p} increases. Amdahl's Law is particularly relevant to this scenario.
...
\begin{figure}[htbp] % h=here, t=top, b=bottom, p=page
  ...
  % Opção 1: Reduzir a largura (tente valores menores que 0.6)
  % \includegraphics[width=0.4\textwidth]{trie.pdf}

  % Opção 2: Controlar largura E altura, mantendo a proporção (RECOMENDADO para figuras altas)
  % A figura será escalada para caber DENTRO de 60% da largura E 50% da altura do texto, sem distorcer.
  % \includegraphics[width=0.6\textwidth, height=0.5\textheight, keepaspectratio]{trie.pdf}
  % Experimente outros valores, ex:
...
\subsubsecao{Failure Function (f)}
...
\subsubsecao{The Output Function}
...
Once the Aho-Corasick automaton is constructed, it processes the input text $T$ character by character in a single pass. Starting from the root state, the automaton iterates over each text character $a_i$ as follows:
\begin{enumerate}
    \item Attempt $\operatorname{goto}(\textit{current\_state}, a_i)$ for the current character $a_i$.
    \item If the transition succeeds, set $\textit{current\_state} \leftarrow s'$.
    \item Otherwise, set $\textit{current\_state} \leftarrow \operatorname{f}(\textit{current\_state})$ and repeat step 1 without consuming $a_i$ until a goto transition succeeds or the root is reached. From the root, $\operatorname{goto}(0, a_i)$ always exists, possibly returning to the root itself.
    \item After each successful transition, inspect $\operatorname{output}(\textit{current\_state})$ and report every pattern it contains, recording its ending position $i$.
\end{enumerate}
The combined number of goto and failure transitions during a scan of length $n$ is $< 2n$ \cite{AhoCorasick1975}.
