...
the end-to-end wall-clock consistency check discussed in the Results chapter.
Those end-to-end figures are not a separate campaign: each is the sum of two
quantities that the canonical sweep already records for the corresponding
execution---the construction time (\texttt{build\_ms}) and a single full-corpus
search pass (\texttt{mean\_ms}). They can therefore be reproduced and audited
from the same database as every other figure.
% epic-03: canonical measurement database moved to the Ryzen 9 9950X workstation sweep.
The benchmark campaign stores its measurements in a single typed SQLite
database located at
\path{parallel-aho-corasick/runs/workstation_2026-06-30/sweep.db},
...
\begin{table}[H]
    ...
    \begin{tabular}{@{} l p{8.8cm} @{}}
        ...
        \texttt{v\_footprint}     & Throughput as a function of the automaton footprint, at one and thirty-two threads. \\ \addlinespace
