...
intrusion detection systems and malware analysis tools, owing to its linear-time
...
which keeps the hot path free of locks; a flattened, contiguous output layout is
further proposed to eliminate the pointer-chasing cost of emitting matches. The
study deliberately targets the memory-bound regime in which the automaton greatly
...
parallelism clearly dominates pattern-set partitioning: search speedups reach up
...
$74.0\%$ and the parallel speedup saturates near $2.85\times$, exposing memory
bandwidth as the physical ceiling. End-to-end accelerations reach up to
