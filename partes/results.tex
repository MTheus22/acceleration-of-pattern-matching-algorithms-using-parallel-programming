...
Performance cores, leaving early finishers idle at the barrier.
Figure~\ref{fig:load_balancing} recalls the three partitioning policies evaluated
in the table.

% review: comment 055 -- this explanatory figure belongs in the proposal chapter;
% moving or redesigning it is deferred to epic-02 task-02. This task keeps the
% empirical framing and makes the i5-specific balancing result explicit.
...
parallelized. Figure~\ref{fig:parallel_bfs} recalls the dependency structure
before the measured build-speedup table.

% review: comment 056 -- this explanatory construction figure is also proposal
% material; its relocation is deferred to epic-02 task-02 while this task keeps
% the build section focused on the measured operational effect.
...
\begin{figure}[H]
\centering
\caption{Visual comparison of text reading traffic: (a) pure text chunking (text parallelism) where each byte is read once, keeping bus load moderate; and (b) two-dimensional composition (chunking + sharding), which multiplies the text reads by $K$ shards and increases memory-bus pressure, reducing speedup.}
